...
serve(async (req) => {
  ...
  try {
    ...
    const participantContext = participants.length > 0 
      ? `\n\n[등장인물]\n이 일기의 주인공들: ${participantNames}\n일기를 작성할 때 이들이 함께한 하루를 자연스럽게 묘사한다.`
      : '';
    
    // 등장인물 정보를 포함한 주인공 표현
    const subjectDescription = participantNames || '집사';

    // System prompt in Korean for diary - 함수로 변경하여 동적으로 생성
    const getPerspectiveMap = () => {
      return {
      camera: {
        name: '핸드폰',
...
- 하지만 결국 집사를 좋아해서 끝은 따뜻하게 마무리한다.${participantContext}
...
2. 사진 속 '확실한 사실'을 바탕으로 ${participantNames || '집사'}의 전체 하루를 묘사한다.
